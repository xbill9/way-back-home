...
def ws_connect(main_module):
    ...
    Teardown suppression is a workaround, not politeness: the endpoint awaits
    `asyncio.gather(upstream_task(), downstream_task(), heartbeat_task())` and
    `heartbeat_task` loops forever, so the gather never completes and the test
    portal cancels it. Delete the suppression once the endpoint shuts down
    cleanly -- it is the canary for that fix.
    """
    import contextlib
    ...
    def _connect(path="/ws/u1/s1"):
        # The suppression must wrap ONLY the teardown. Wrapping the `yield` --
        # e.g. `with contextlib.suppress(Exception), client.websocket_connect(...)`
        # -- makes @contextmanager swallow anything raised in the caller's body,
        # so every assertion inside a `with ws_connect()` block passes vacuously.
