...
def test_model_audio_is_sent_as_binary_not_base64(main_module, monkeypatch, ws_connect):
    """Model audio rides its own prefixed binary frame, and only that frame.

    It used to travel as base64 inside the event JSON, which inflates binary by
    a third: measured over a 14s session, the downlink was 220KB of which 202KB
    was base64 audio -- 51KB of pure encoding overhead, 23% of everything sent
    to the browser. The regression to guard against is subtle in both
    directions: sending it twice (binary *and* still in the JSON) doubles the
    cost silently, and dropping the binary frame leaves the demo mute.
    """
    from google.adk.events import Event
    from google.genai import types
