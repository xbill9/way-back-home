...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=0.0,
        help=(
            "stop sending frames for this many seconds after the stimulus, then "
            "resume. Tests whether a frame landing while the model is answering "
            "is what makes it call report_digit again -- duplicates arrive ~0.65s "
            "apart, which is one frame at 1 FPS. 0 (default) streams throughout"
        ),
    )
    p.add_argument(
        "--burst",
        ...
        default=0,
        help=(
            "send this many frames then the stimulus and no more video until "
            "the next trial, instead of streaming for the whole hold. Models "
            "the client sending video only in the wake-word burst; 0 (default) "
            "keeps the continuous 1 FPS stream the browser sends today"
        ),
    )
    p.add_argument(
        "--jpeg-quality",
    ...
    async with websockets.connect(url, max_size=None) as ws:
        ...
        for rnd in range(args.rounds):
            for digit in digits:
                ...
                async def send_frame(digit=digit):
                ...
                if args.burst:
                    # Burst mode: a short run of frames, the stimulus, then no
                    # video at all until the next trial -- the client's
                    # wake-word burst without the 1 FPS stream underneath it.
                    # The point is to leave the model nothing new to re-analyse
                    # once it has answered, which is when walkie-talkie
                    # re-calls report_digit and buries its own confirmation.
                    for _ in range(args.burst):
                        await send_frame()
                        frames += 1
                        await asyncio.sleep(interval)
                    stimulus_at = await send_stimulus()
                    await asyncio.sleep(max(0.0, deadline - time.monotonic()))
                else:
                    while time.monotonic() < deadline:
                        answering = (
                            stimulus_at is not None
                            and time.monotonic() - stimulus_at < args.pause_after_scan
                        )
                        if not answering:
                            await send_frame()
                            frames += 1
