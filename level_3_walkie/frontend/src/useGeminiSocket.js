...
export function useGeminiSocket(
  ...
  // While this is in the future, the capture loop keeps ticking but sends
  // nothing -- see captureOnce. Set when a scan goes out, cleared the moment
  // the model starts speaking, so video resumes as soon as the answer begins
  // rather than waiting out the cap.
  const holdFramesUntil = useRef(0);
  // The phrase the wake word matched on, carried to the backend for diagnosis.
  const lastHeard = useRef("");
  const HOLD_FRAMES_MS = 6000;
  ...
  const startStream = useCallback(
    async (videoElement) => {
      try {
        ...
        if (micMode === "wake" && wakeWordSupported()) {
          try {
            ...
            scanScheduler.current = new ScanScheduler({
              send: () => {
                if (ws.current?.readyState !== WebSocket.OPEN) return;
                // `heard` is diagnostic only -- the backend logs it and acts on
                // `text`. It answers a question the server otherwise cannot:
                // whether a scan came from you saying "scan" or from the
                // scanner's own "Scanner Online." reaching the microphone.
                // `wakeWord.js` matches `scan` as a substring, so "scanner"
                // matches, and the model says it out loud.
                ws.current.send(
                  JSON.stringify({ type: "text", text: "scan", heard: lastHeard.current }),
                );
                holdFramesUntil.current = performance.now() + HOLD_FRAMES_MS;
              },
            ...
            wakeWord.current = new WakeWordListener(undefined, {
              onCommand: (heard) => {
                ...
                if (alwaysInterrupt) {
                  ws.current.send(
                    JSON.stringify({ type: "text", text: "scan", heard }),
                  );
                  outcome = "sent";
                } else {
                  outcome = scanScheduler.current.request();
                }
                pushEvent(
                  "you",
        ...
        const captureOnce = (force = false) => {
          // Hold frames while the model is answering a scan. Duplicate
          // `report_digit` calls arrive ~0.65s apart -- one frame at 1 FPS --
          // and each one delays the spoken confirmation by about that much,
          // because BLOCKING declarations mean the model may not speak until
          // every call has round-tripped. A frame landing mid-answer is a fresh
          // reason to call again. Measured over 10 scans: worst confirmation
          // 4.08s -> 2.48s, uplink 128.5 -> 96.6 kbit/s, accuracy unchanged at
          // 10/10.
          //
          // Only frames AFTER the request are held, never the ones before it.
          // That distinction is the whole reason this is safe where per-scan
          // sessions were not: the model keeps every frame it had when asked,
          // and starving it of that context is what wrecked recognition then.
          if (!force && performance.now() < holdFramesUntil.current) return;
        ...
        // `force`: the burst is the pose being asked about, so it goes out
        // even when the loop is holding frames. Without this a scan arriving
        // during a previous scan's hold -- exactly what happens after a storm,
        // when the hold runs its full length -- would send the request with no
        // fresh frames behind it, which is how the model ends up answering with
        // the previous hand.
        captureBurstRef.current = (count = 2, gapMs = 120) => {
          for (let i = 0; i < count; i++) setTimeout(() => captureOnce(true), i * gapMs);
        };
