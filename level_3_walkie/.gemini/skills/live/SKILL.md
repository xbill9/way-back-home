...
- `models/walkie-talkie`, `models/clever-chatter` — Early Access Program, allowlisted per GCP project. Not in this reference and not publicly documented; the deltas from 3.1 Flash Live are in [Live audio EAP](#live-audio-eap-walkie-talkie--clever-chatter) below. **This project runs `models/walkie-talkie`**, so where the two disagree, that section wins.
...
## Live audio EAP: walkie-talkie / clever-chatter

Everything above describes `gemini-3.1-flash-live-preview`. The EAP models
change enough of it that the differences are worth having in one place. Source:
the EAP invitation and its `interaction_status` follow-up — no public docs
exist. This project's own notes are in `GEMINI.md` → "Live audio EAP".
...
Corrections to the sections above, for these models only:

- **Async function calling** is supported — it is the *default*, and the only
  mode `clever-chatter` accepts. This inverts the "synchronous only" note under
  [Limitations](#limitations).
- **Proactive audio** is permanently enabled. Sending `proactiveAudio: false`
  is a hard error, not a no-op.
- **Affective dialogue** is removed from the API entirely, not merely
  unsupported.
- **Client content** — `send_client_content` works for the whole session, not
  just for seeding history. With `turn_complete=true` it unconditionally
  interrupts generation; realtime text instead respects VAD and defers to the
  end of speech. Migration note 4 above is superseded.
- **Response modality** — AUDIO only. Use output audio transcription for text.
- **`turnComplete: true` no longer means idle.** Tool calls and audio frames
  can follow it. `interaction_status` (`IN_PROGRESS` / `REQUIRES_ACTION`) is
  the definitive signal, but it ships only in a private `google-genai` build
  and ADK drops it, so ADK applications cannot read it today.
