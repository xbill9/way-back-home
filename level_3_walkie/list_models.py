...
# EAP endpoints are not returned by models.list(), so their absence here says
# nothing about whether this key can reach them. The one-call check for that is
# `python -m pytest test_live_connection.py -s` (billed).
print(
    "\nNote: models/walkie-talkie and models/clever-chatter are EAP endpoints "
    "and are not expected to be listed above."
)
